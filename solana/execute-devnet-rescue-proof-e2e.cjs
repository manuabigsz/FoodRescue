...
async function main() {
  const prepared = await request(`/trades/${trade.trade_id}/rescue-proof/prepare`, 'POST');
  ...
  const confirmed = await request(`/trades/${trade.trade_id}/rescue-proof/confirm`, 'POST', { signature, proof_pda: proofPda.toBase58() });
