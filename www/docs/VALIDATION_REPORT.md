...
Seeds verificadas no Rust: `foodrescue_protocol + authority`; `foodrescue_trade + trade_id_le + buyer`; `foodrescue_vault + trade_id_le + buyer`; `foodrescue_rescue + trade_id_le + ngo + producer`. O backend verifica owner/layout/termos do PDA, mas não recalcula independentemente o bump usando uma biblioteca de curva: a derivação é validada pelo programa.
...
1. **Autorização das seeds de trade/proof — revisto nesta rodada.** A rodada anterior registrou a exigência da assinatura da authority em `initialize_trade`, mas isso existia apenas na preparação do Laravel: o `processor.rs` nunca leu essa conta, então o payload e o programa divergiam e nenhuma transação de criação de custódia teria sido aceita como descrita. A proteção foi reimplementada de outra forma: a carteira do buyer entra nas seeds do trade e da vault (`foodrescue_trade + trade_id_le + buyer`). Como `initialize_trade` já exige `buyer.is_signer`, ocupar o PDA de outra pessoa passa a exigir a assinatura dela — antecipar um ID sequencial deixa de bloquear a operação alheia. O ganho adicional é operacional: a instrução continua com um único signatário, então a carteira do navegador consegue enviá-la sem chave privada no servidor. `create_rescue_proof` recebeu o mesmo tratamento: NGO e producer entram nas seeds do `foodrescue_rescue` e a authority deixou de ser signatária. Como duas assinaturas na mesma transação não sobrevivem a dois atores assinando em momentos diferentes — o `recentBlockhash` expira antes —, a atestação passou a ser feita em duas instruções: a NGO abre com a tag 5 e o producer fecha com a nova tag 9, cada um assinando sozinho. O `RescueProofState` ganhou um byte de status (versão 2, 147 bytes) e o trade só vai a `COMPLETED` quando o estado on-chain estiver `CONFIRMED`. Trades e proofs já criados on-chain sob as seeds antigas não são migrados: exigem redeploy e recriação.
